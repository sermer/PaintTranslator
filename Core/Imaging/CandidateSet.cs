...
namespace PaintTranslator.Imaging
{
    ...
    public sealed class CandidateSet
    {
        ...
        /// <summary>
        /// Gets the largest CIELAB C*ab in this immutable set. Computing it once at
        /// construction avoids rescanning a cached 84k-colour gamut for every preview
        ...
        private static double FindMaximumChroma(double[] a, double[] b)
        {
            double largestSquared = 0.0;
            for (int i = 0; i < a.Length; i++)
            {
                double squared = (a[i] * a[i]) + (b[i] * b[i]);
                largestSquared = Math.Max(largestSquared, squared);
            }
